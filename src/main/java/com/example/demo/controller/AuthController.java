...
import com.example.demo.model.payload.response.JwtResponse;
import com.example.demo.model.payload.service.UserDetailsImpl;
...
public class AuthController {
	...
	public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
		...
		logger.info("Step 1 of execution");
		
		Authentication authentication = authenticationManager.authenticate(
				new UsernamePasswordAuthenticationToken(loginRequest.getUsername(), loginRequest.getPassword()));

		
		logger.info("Step 2 of execution");
	...
	public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
		...
		if (userRepository.existsByEmail(signUpRequest.getEmail())) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiResponse(HttpStatus.BAD_REQUEST.value(),
					"Error: Email id is already in use!", "Error: Email id is already in use!"));
		}
